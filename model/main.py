...
#     def __init__(self, input_dim, output_dim, device, A_plus=0.02, A_minus=0.02, tau_plus=20.0, tau_minus=20.0):
...
#         # pre_spike: [B, input_dim]
#         # post_spike: [B, output_dim]
#         # delta_t: [B,1]
#
#         post_spike_expanded = post_spike.unsqueeze(-1)  # [B, output_dim, 1]
#         pre_spike_expanded = pre_spike.unsqueeze(-2)    # [B, 1, input_dim]
#         delta_t_expanded = (delta_t / self.tau_plus).unsqueeze(-1)  # [B,1,1]
#
#         delta_w_pot = self.A_plus * post_spike_expanded * pre_spike_expanded * torch.exp(-delta_t_expanded)
#         delta_w_dep = self.A_minus * post_spike_expanded * pre_spike_expanded * torch.exp(-delta_t_expanded / self.tau_minus)
#
...
def train_one_epoch(
    ...
    # 如果使用DDP，model_without_ddp即model.module
    # 若非DDP则model_without_ddp=model本身
    # 请在调用此函数前根据实际情况获取model_without_ddp
    # 例如在 main中:
    ...
    for idx, (image, target) in enumerate(data_loader_train):
        ...
        with GlobalTimer('iter', timer_container):
            if scaler is not None:
                with autocast():
                    output = model(image) # [B,C]的logits输出，不做mean(0)等操作
            ...
            # 在scheduler_per_iter存在的情况下更新scheduler
            if scheduler_per_iter is not None:
                scheduler_per_iter.step()

            # 完成梯度更新后，对SSD进行无监督权重更新（若满足条件）
            # 确保在 forward 中已将分类器输入特征保存在 model_without_ddp.out
            # 并且未在 forward 内做SSD更新
            # 此时 pre_spike 和 post_spike 不干扰梯度流

            model_without_ddp = model.module if hasattr(model,'module') else model
            if hasattr(model_without_ddp,
                       'current_epoch') and model_without_ddp.current_epoch >= model_without_ddp.start_epoch:
                pre_spike = (model_without_ddp.out > 0).float().detach()  # [B,C_in]
                post_spike = (output > 0).float().detach()  # [B,C_out]
                ...
                with torch.no_grad():
                    ...
                    if delta_w.shape != model_without_ddp.classifier.weight.shape:
                        print("[SSD Debug]: delta_w and classifier.weight shape mismatch")
